...
jest.mock('@/design_system/components/atoms/ThemedText', () => {
  ...
  // Add this mock for ChatSelector
  jest.mock('@/design_system/components/organisms/ChatSelector', () => {
    const React = require('react');
    const { View } = require('react-native');
    return {
      ChatSelector: ({ visible, onClose, onChatSelected }: 
        { visible: boolean; onClose: () => void; onChatSelected: (chatId: string) => void }) => 
        <View testID="chat-selector" />,
    };
  });
...
describe('MessageBubble Component', () => {
  ...
  it('applies different styles for current user vs other users', () => {
    ...
    expect(selfMessageContainer).toBeTruthy();
    ...
    expect(otherMessageContainer).toBeTruthy();
