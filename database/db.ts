 ...
 */
// Update searchMessages function to handle new fields

const limit = 50; // Number of messages to retrieve per paginatio
export async function searchMessages(searchTerm: string, userId: string, offset: number) {
  try {
    console.log('Pagination params:', { searchTerm, userId, limit, offset });
    ...
    const query = `
        SELECT 
          ...
          json_extract(m.forwarded_from, '$.userId') AS forwarded_user_id,
          json_extract(m.forwarded_from, '$.userName') AS forwarded_user_name,
          json_extract(m.forwarded_from, '$.originalTimestamp') AS forwarded_timestamp
        FROM messages m
    ...
    const params = [userId, userId, userId, `%${searchTerm}%`, limit, offset];
